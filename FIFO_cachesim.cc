...
// get sets, ways, linesz from config
...
{
  if (sets == 0 || (sets & (sets - 1))) // if set == 0 or not 2^n
    help();
  if (linesz < 8 || (linesz & (linesz - 1))) // if linesz < 8 || not 2^n
    help();
  ...
  victim_idx = new uint64_t[sets]();
  std::fill_n(victim_idx, sets, 0);
}
...
// check tag funcion for checking
...
// function for store in cache and find victim
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets - 1);

  // size_t way = lfsr.next() % ways;   // original random way

  // for finding victim
  uint64_t victim = tags[idx * ways + victim_idx[idx]];

  // for storing addr
  tags[idx * ways + victim_idx[idx]] = (addr >> idx_shift) | VALID;

  // update index
  victim_idx[idx] = (victim_idx[idx] + 1) % ways;
