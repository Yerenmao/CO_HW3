...
void cache_sim_t::init()
{
  ...
  clock = 0;
  table = new uint64_t[sets * ways]();
  std::fill_n(table, sets * ways, 0);
}
...
// check tag function for checking
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  uint64_t victim_time = std::numeric_limits<uint64_t>::max();
  for (size_t i = 0; i < ways; i++)
  {
    if (table[idx * ways + i] <= victim_time)
    {
      victim_index = i;
      victim_time = table[idx * ways + i];
    }
  }
